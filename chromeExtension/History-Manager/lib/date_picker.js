...
BeatPicker.prototype = {
    ...
    className: {
        ...
        selected: "selected",
        disabled: "disabled",
        clear: "beatpicker-clear btn btn-info btn-margin",
        _inner: {
            beatPicker: "beatpicker",
    ...
    aa: {},
    constructor: function() { ! window.beatPickerList && (window.beatPickerList = []);
    ...
    al: function(a) {
        ...
        a, this.labels.gotoDateButton).on("click",
        function() {
            b.cg(c)
        })
    },
    am: function() {
        ...
        if (this.view.display === this.x.days) {
            ...
            for (c = 0; 42 > c; c++) {
                ...
                d.on("click",
                function(b) {
                    var c = $(this).data("click-behaviour"); ! $(this).data("disabled") && a.au(b);
    ...
    an: function() {
        ...
        for (var b = 0; 11 >= b; b++) {
            ...
            this._daysCellParent, this.monthsSimple[b]).on("click",
            function() {
    ...
    ao: function() {
        ...
        for (var b = this.ce(this.currentDate.getFullYear()), c = 0; 11 >= c; c++) {
            ...
            this._daysCellParent, b[c]).on("click",
            function() {
    ...
    ar: function() {
        var a = this;
        this.view.alwaysVisible || (a.k.css("display", "none"), $("html,body").on(this.view.hideOn,
        function(b) { ! a.v && a.hide()
        }), this.k.on("click",
        function(a) {
            a.stopPropagation()
        }));
        ...
        b.on(this.view.showOn,
        function(b) {
            b.stopPropagation();
            if (a.v || a.selectionRule.range) {
                var d = $.grep(window.beatPickerList,
                function(b, c) {
                    return b !== a && !b.view.alwaysVisible
                });
                a.forEach(d,
                function(a, b) { ! b.isHide() && b.hide()
                },
                a);
                a.show(b)
            }
    ...
    av: function(a, b) {
        if (a) if (b === this.y.briefDate)(a || this._currentIndicator).text(this.monthsFull[this.currentDate.getMonth()] + " " + this.currentDate.getFullYear());
            ...
            var c = this.ce(this.currentDate.getFullYear()); (a || this._currentIndicator).text(c[0] + "-" + c[c.length - 1])
        }
    },
    aw: function(a, b, c) {
        var d = {
    ...
    ay: function(a) { ! a && this.cq(this.events.change, {
            date: this.currentDate,
    ...
    bf: function(a) {
        ...
        g = "";
        this.forEach(this.dateFormat.format,
        function(a, b) {
            e[b] && (g += e[b] + (2 > a ? f: ""))
        },
        this);
    ...
    bk: function() {
        for (var a in this.disablingRules) {
            ...
            if (isNaN(Date.parse(b.to)) && !d instanceof Array && !d in this.z) break; (b = this.bl(b.from, b.to)) && this.w.push(b)
        }
    },
    bl: function(a, b) {
    ...
    bm: function(a) {
        var b = {};
        a instanceof Array && (b.date = a[2], b.month = isNaN( + a[1]) ? a[1] : +a[1] - 1, b.year = a[0]);
    ...
    bz: function() {
        var a = this;
        this.on(this.events.hide,
        function(b) { (a.m || a.n) && a.ax(a.m || a.n, !0);
            a.az(new Date)
        })
    },
    ca: function(a) {
    ...
    show: function(a) {
        this.k.css("display", "");
        this.v = !1; ! this.view.alwaysVisible && this.cq(this.events.show);
