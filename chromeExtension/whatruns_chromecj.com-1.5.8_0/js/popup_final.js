var currentHostName = '', detectedApps = {};
(function() {
		...
		var popup = {
			...
			setEvents : function(){
				var sendMessage = function(tabs){
					BROWSER.runtime.sendMessage({ id: GET_DETECTED_APPS, tab: tabs[0] }, function(res){
			...
			getAppsByDomainName : function(tabs){
				try{
					var sendMessage = function(tabs){
					   BROWSER.runtime.sendMessage({ id: GET_HOST_NAME, tab: tabs[0] }, function(res){
			...
			getSiteData : function(){
				try{
					var sendMessage = function(tabs){
					   BROWSER.runtime.sendMessage({ id: GET_SITE_DATA, tab: tabs[0] }, function(res){
			...
			analyseSiteData : function(data){
				try{
					 ...
					 xmlhttp.onreadystatechange = function(e) {
						 if ( xmlhttp.readyState == 4 && xmlhttp.status==200) {
							 try{
								 var response = xmlhttp.responseText, res = {};
			...
			getCurrentHostName : function(data){
				try{
					if(typeof data.subdomain != "undefined") return data.rawhostname;
					else if(typeof data.tabTechs.subdomain != "undefined") return data.tabTechs.rawhostname;
					...
					if(typeof response.tabTechs.fontFamily != "undefined"){
						...
						try{
							$(oddEl).append(
								...
								.append(
									$('<div>', {"class" : "category-name"}).text("Font")
								)
								.append(
									$('<div>', {"class" : "border"})
								)
								.append(
									$('<div>', {"class" : "current-tech-info sub-element capitalize"})
									.append(
										$('<span>', {"class" : "label label-default"}).text("Font")
									)
									.append(
										$('<span>', {"class" : "themes-list"}).text(font)
									)
								)
								.append(
									$('<div>', {"class" : "current-tech-info sub-element capitalize"})
									.append(
										$('<span>', {"class" : "label label-default"}).text("Font Family")
									)
									.append(
										$('<span>', {"class" : "themes-list"}).text(fontFamily)
									)
								)
							);
						...
						self.removeAppsDiv();
						$(techInfoEl).append(
							$('<div>', {"class" : "techs-list text-center no-apps"})
							.append($('<div>', {"class" : "category-name no-apps"})).text(NO_APPS_FOUND)
						);
					}
			...
			appendAppDiv : function(el, categoryName, appsArr, categoryClass){
				$(el).append(
								$('<div>', {"class" : "techs-list " + categoryClass})
								.append($('<div>', {"class" : "category-name"}).text(categoryName))
							...
							try{
								...
								var techListEl = '';
								var length = 0;
								var techsListArr = [], isWordPressDisabled = false;
								...
								for(var i in arr){
									...
									if(typeof techJson.parentElement == "undefined"){
										...
										techListEl = $('<div>', {"class" : "current-tech-info"}
										).append(
											$('<img>', {"class" : "icon", "src" : imageUrl})
										).append(
											$('<div>', {"class" : "tech-name"}).text(techName)
										).append(
											$('<a>', {"href" : siteListUrl, "target" : "_blank", "class" : "techs-list-url", "data-toggle" : "tooltip", "data-placement" : "bottom", "title" : "Know More"})
											.append($('<img>', {"src" : "images/details.png"}))
										).append(
											$('<a>', {"href" : href, "target" : "_blank", "class" : "original-site", "data-toggle" : "tooltip", "data-placement" : "bottom", "title" : "Website"})
								...
								if(!isWordPressDisabled){
								   if(themes.split(",").length > 1){
									   $(el).append(
										   $('<div>', {"class" : "current-tech-info sub-element"})
										   .append(
											   $('<span>', {"class" : "label label-default"}).text("Theme")
										   )
										   .append(
											   $('<span>', {"class" : "themes-list break-word"}).text(self.removeLastComma(themes))
										   )
									   );
								   }
								   if(plugins.split(",").length > 1){
									   $(el).append(
										   $('<div>', {"class" : "current-tech-info sub-element"})
										   .append(
											   $('<span>', {"class" : "label label-default"}).text("Plugins")
										   )
										   .append(
											   $('<span>', {"class" : "plugins-list break-word"}).text(self.removeLastComma(plugins))
										   )
									   );
								   }
								}
