var email = null, api_key = null, followingSites = [], lastContainer = '';
var loginKey = "login_session", followingSitesKey = "following_sites";
	...
	var popupExt = {
		...
		initBackEvent : function(){
			$('.btn-back').click( function(e){
			   ...
			   if(lastContainer.trim().length >0){
				   ...
				   $(lastContainer).addClass('active');lastContainer = '';
			   }else{
			   ...
			   if(typeof response != "undefined"){
				   if(typeof response.following_sites != "undefined"){
					   ...
					   Object.keys(following_sites).forEach( function(siteName){
						   var siteDetectedApps = JSON.parse( following_sites[siteName] );
		...
		formSubmit : function(){
			$('.form').on("submit", function(e){
				...
				var $this = $(this);
				$('.result').empty();$('.result').removeClass("error");
				$.ajax({
					type : method,
					data : data,
					url : DOMAIN_NAME + url,
					success : function(res){
						if(res.status != 200){
							...
							if(typeof res.api_key != "undefined") current.setLoginData(res);
						}
					}
				});
			});
